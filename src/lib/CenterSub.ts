...
import { CashedMessageEntry, MessagesStorage } from './storage';
import { Logger } from '../utils/logger';
import { outboundStreamDelay } from './constants';

const logger = Logger('CenterSub');
...
export class CenterSub extends GossipSub {
  ...
  private messages = new MessagesStorage();
  ...
  private async publishToPeer(peerId: PeerId, messages: CashedMessageEntry[]): Promise<void> {
    ...
    logger.debug('publishToPeer: peerId:', id);

    if (!this.peers.has(id)) {
      logger.debug('publishToPeer: peers.has(id):', false);
      return;
    }

    if (messages.length === 0) {
      logger.debug('publishToPeer: messages.length:', 0);
      return;
    }
    ...
    logger.debug('publishToPeer: sendRpc:', sentMsgIds, sent);
  }
  ...
  private async cacheMessage(rpcMsg: RPC.IMessage): Promise<void> {
    try {
      if (!rpcMsg.from || !rpcMsg.data) {
        logger.debug('Anonymous message');
        return;
      }
      if (!this.messageTransformer) {
        logger.debug('messageTransformer not defined');
        return;
      }
  ...
  private async handlePeerConnect(peerId: PeerId): Promise<void> {
    try {
      ...
      logger.debug('handlePeerConnect: missedMessages.length:', missedMessages.length);
